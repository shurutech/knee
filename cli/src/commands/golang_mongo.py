...
class GolangMongo(Golang, Mongodb):
    def __init__(self, environment="staging", config_dir="playbooks/group_vars"):
        ...
        self.hosts = self.file_manager.read_from_file(
            DIRECTORY_PATH[environment], "hosts.yml"
        )
        ...
        self.server = Golang(environment)
    ...
    def write_configuration_and_run_playbook(self):
        self.file_manager.write_to_file(
            DIRECTORY_PATH[self.environment], "hosts.yml", self.hosts
        )
        for config_file in CONFIG_FILES:
            self.file_manager.write_to_file(
